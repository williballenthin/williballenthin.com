...
You can use miasm's 
 [architecture abstraction](https://github.com/cea-sec/miasm/blob/a2d2e3bd87636c2ccca1afecc5c7c20360aa43fa/miasm/arch/x86/arch.py)
 to parse bytes into a single instruction instance.
Note that only the first instruction is parsed.

```python
import miasm.arch.x86.arch
insn = miasm.arch.x86.arch.mn_x86.dis(b'\xB8\x01\x00\x00\x00\x90\x90\x90\x90', mode_o=32)
...
machine = miasm.analysis.machine.Machine('x86_32')
